...
import { CheckCircle2, Clock, AlertCircle, PlayCircle, Send, Search } from "lucide-react";
...
export default function KanbanBoard({ title, isAssignedToMe = false }: any) {
  ...
  const briefs = isAssignedToMe
    ? store.briefs.filter((b: any) => {
      // Admins should see all tasks that are pending review ("uploaded")
      if (currentUser.role === 'admin' && b.status === 'uploaded') return true;
  ...
  const cols = [
    ...
    { id: "qa", title: "Send to QA", icon: <Search size={16} />, color: "text-[#0ea5e9]" },
  ...
  return (
    <div className="w-full mx-auto h-full flex flex-col px-8 ">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 xl:gap-6 pb-2 items-start flex-1 min-h-[500px]">
        {cols.map(col => {
          const colBriefs = briefs.filter((b: any) => b.status === col.id);
          return (
            <div
              key={col.id}
              className="kanban-col bg-panel-2/30 border border-line rounded-2xl p-3 min-h-[420px] transition-all flex flex-col"
