...
export default function TaskDetailPage() {
  ...
  const [uploadData, setUploadData] = useState({
    files: [] as { fileObj: File, fileUrl: string, fileName: string }[],
  ...
  useEffect(() => {
    if (brief && !String(brief.id).startsWith('b')) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFiles.length > 0) {
      ...
      setUploadData(prev => ({ ...prev, files: [...prev.files, ...newFiles] }));
    }
  };
  ...
  const handleSubmitWork = async () => {
    if (uploadData.files.length === 0 && !uploadData.liveLink && !uploadData.docContent.replace(/<[^>]*>/g, '').trim()) {
    ...
    try {
      ...
      if (token && !String(brief.id).startsWith('b')) {
        ...
        if (uploadData.files.length > 0) {
          uploadData.files.forEach(f => formData.append("files", f.fileObj));
        }
      ...
      const newUpload = backendSubmission ? {
        ...
        id: `up_${Date.now()}`,
        briefId: brief.id,
        files: uploadData.files.length > 0 ? uploadData.files.map(f => ({ url: f.fileUrl, platform: uploadData.platform, dimensions: "1080x1080", name: f.fileName, type: "image/png" })) : [],
      ...
      setUploadData({ files: [], platform: safePlatforms[0] || "Instagram Feed", note: "", liveLink: "", docContent: "" });
    } catch (err) {
      console.error(err);
    ...
    <div className="flex flex-col px-6" style={{ height: '100%' }}>
      ...
      <div className="flex items-start sm:items-center justify-between gap-3 mb-5 pb-4 border-b border-line shrink-0 flex-wrap">
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.back()}
            className="w-9 h-9 rounded-xl border border-line bg-panel hover:bg-panel-2 flex items-center justify-center text-muted hover:text-text transition-all shrink-0"
          >
            <ArrowLeft size={17} />
          </button>
          <div>
            <div className="text-[10px] font-extrabold text-muted uppercase tracking-widest">Task Detail</div>
            <h1 className="text-lg sm:text-xl font-extrabold text-text m-0 leading-tight">{brief.title}</h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className={`px-3 py-1.5 rounded-xl text-[10px] font-extrabold uppercase tracking-wider border ${getStatusColor(brief.status)}`}>
            {getStatusLabel(brief.status)}
          </span>
        </div>
      </div>

      {/* ── Body: left scrolls, right is fixed ── */}
      <div className="task-page-body">

        {/* ── Left Column (scrollable) ── */}
        <div className="task-left-col">
          <div className="flex flex-col gap-5">
            <div className="bg-panel border border-line rounded-2xl p-5 md:p-7 shadow-sm">
              {/* Meta grid */}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-5 gap-x-6 mb-8 text-sm">
                ...
                <div>
                  ...
                  <span className={`px-2.5 py-1.5 rounded-[6px] text-xs font-extrabold uppercase tracking-wider ${brief.priority === 'Low' ? 'bg-ok/10 text-ok border border-ok/20' :
                      ...
                      <div key={upload.id || idx} className={`border rounded-2xl p-6 ${upload.status === 'approved' ? 'bg-ok/5 border-ok/30' : upload.status === 'revision' ? 'bg-danger/5 border-danger/30' : 'bg-panel-2/50 border-line'}`}>
                        <div className="flex items-center justify-between mb-5 flex-wrap gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            <span className={`px-2 py-1 rounded-md text-[10px] font-extrabold uppercase tracking-wider border ${upload.status === 'approved' ? 'bg-ok/10 text-ok border-ok/20' :
        ...
        {/* ── Right Column: Live Chat (fixed height) ── */}
        <div className="task-right-col overflow-hidden rounded-2xl border border-line shadow-sm bg-panel">
          {/* Chat Header */}
          <div className="flex items-center gap-3 px-4 py-3 border-b border-line bg-panel shrink-0">
            <div className="relative shrink-0">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-[#14a879] flex items-center justify-center text-white text-base font-black shadow-sm">💬</div>
              <div className="absolute bottom-0 right-0 w-3 h-3 rounded-full bg-ok border-2 border-panel" />
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="font-extrabold text-sm text-text leading-tight">Live Chat</h3>
              <p className="text-[10px] text-ok font-semibold">● Online · visible to all members</p>
            </div>
          </div>
          {/* Chat body fills remaining height */}
          <div className="flex-1 overflow-hidden flex flex-col min-h-0">
            <TaskComments taskId={brief.id} className="flex flex-col h-full" />
          </div>
        </div>
      ...
      <Modal isOpen={isUploadModalOpen} onClose={() => setIsUploadModalOpen(false)} title={brief.status === 'revision' ? '↻ Resubmit Work for Review' : '✦ Submit Work for Review'} maxWidth="max-w-2xl">
        <div className="flex flex-col">
          <div className="overflow-y-auto max-h-[75vh]">
            {/* Role + Status Banner */}
            <div className={`mx-6 mt-4 flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold border ${currentUser.role === 'designer'
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="relative border-2 border-dashed border-line rounded-xl p-6 text-center hover:bg-panel-2 transition-colors cursor-pointer group">
                  ...
                  <div className="flex flex-col items-center gap-2 pointer-events-none">
                    ...
                    <p className="text-sm font-bold text-text">Click or drag files to upload</p>
                    <p className="text-xs text-muted font-medium">Supports multiple JPG, PNG, MP4, PDF</p>
                  </div>
                </div>

                {uploadData.files.length > 0 && (
                  <div className="flex flex-col gap-2 mt-2">
                    <p className="text-[11px] font-extrabold text-muted uppercase tracking-widest">Selected Files ({uploadData.files.length})</p>
                    <div className="grid grid-cols-2 gap-3">
                      {uploadData.files.map((f, idx) => (
                        <div key={idx} className="relative group/file bg-panel-2 border border-line rounded-lg overflow-hidden flex items-center p-2 gap-2">
                          {f.fileObj.type.startsWith('image/') ? (
                            <img src={f.fileUrl} alt={f.fileName} className="w-8 h-8 object-cover rounded shadow-sm shrink-0" />
                          ) : (
                            <div className="w-8 h-8 bg-panel border border-line rounded flex items-center justify-center shrink-0">
                              <ImageIcon size={14} className="text-muted" />
                            </div>
                          )}
                          <span className="text-[11px] font-bold text-text truncate pr-4" title={f.fileName}>{f.fileName}</span>
                          <button
                            type="button"
                            onClick={() => removeFile(idx)}
                            className="absolute right-1.5 top-1/2 -translate-y-1/2 text-danger opacity-0 group-hover/file:opacity-100 p-1 hover:bg-danger/10 rounded transition-all z-20"
                          >
                            <XCircle size={14} />
                          </button>
                        </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {safePlatforms.map((p: string) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${uploadData.platform === p
